...
def patient_kfold_indices(names, k=5, seed=42):
    ...
    for tr_pat_idx, val_pat_idx in kf.split(patients):
        train_idx, val_idx = [], []
...
class JointHeatmapS2(nn.Module):
    def __init__(self, n_landmarks=9, dropout=0.3):
        super().__init__()
    ...
    def forward(self, patches_list):
        ...
        _, l2_features = self.sa2(l1_xyz, l1_features)
        ...
        for k in range(self.n_landmarks):
            ...
            global_expanded = global_k.unsqueeze(2).expand(-1, -1, n_patch)
...
def joint_loss(preds, scores_list, l1_xyz_list, residuals_batch, sigma):
    ...
    for k in range(N_LANDMARKS):
        ...
        loss += F.smooth_l1_loss(preds[k], residual)
        loss += LAMBDA_H * gaussian_heatmap_kl(scores_list[k], l1_xyz_list[k], residual, sigma)
...
def fps(points, n_points, seed=None):
    if DEVICE.type == "cuda" and points.shape[0] >= n_points:
        try:
            ...
            idx = furthest_point_sample(points_t, n_points)
...
def closest_on_triangles_batch(point, v0, v1, v2):
    ...
    if mask_ab.any():
        den = d1[mask_ab] - d3[mask_ab]
        s = np.where(np.abs(den) > 1e-30, np.clip(d1[mask_ab] / den, 0, 1), 0.0)
    ...
    if mask_ac.any():
        den = d2[mask_ac] - d6[mask_ac]
        w = np.where(np.abs(den) > 1e-30, np.clip(d2[mask_ac] / den, 0, 1), 0.0)
...
def load_data():
    ...
    X_list, Y_list, scale_list = [], [], []
    names, full_clouds, triangles_list = [], [], []
    n_with_mesh = 0
...
class JointPatchDataset(Dataset):
    def __init__(self, X, Y, coarse, radii, jitter, mix_prob=0.5, training=True):
        self.X = X
        self.Y = Y
        self.coarse = coarse
        self.radii = radii
